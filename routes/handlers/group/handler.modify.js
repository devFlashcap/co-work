...
module.exports.group_modify = async (store, io, socket, data) => {
    ...
    if(!isValid){
        ...
        const group = await dm_group.group_modify({
        ...
        if(group.status === response_code.HTTP_404){
            ...
            const notification = {
                ...
                message: 'Modifications has been made to group: ' + group.response.name,
            };
            ...
            for(const onlineMember of onlineMembersNotInRoom){
                ...
                if(notificationSaved.status === response_code.HTTP_200){
                    for(const socketID of onlineMember.socketIDs){
                        ...
                        socket.to(socketID).emit('action', {type: actions.GROUP_MODIFY, payload: group.response});
                    }
                }
            }
            ...
            socket.emit('action', {type: actions.FORM_RESULTS_SET, payload: {results: "Group modification successful", success: true}});
