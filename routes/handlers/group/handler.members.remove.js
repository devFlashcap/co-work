...
module.exports.group_members_remove = async (store, io, socket, data) => {
    const group = await dm_group.group_members_remove({
    ...
    if(group.status === response_code.HTTP_200){
        ...
        const notification = {
            ...
            type: 'chat',
            reference: null,
            message: 'You have been removed from the group: ' + group.response.name,
        };
